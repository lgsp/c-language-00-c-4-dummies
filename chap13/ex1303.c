...
the separation.";
  ...
  int index, alpha, blank, punct, lower, upper, printable;

  alpha = blank = punct = lower = upper = printable = 0;
  ...
  while(phrase[index])
  {
    ...
    if(isgraph(phrase[index])) printable++;
    index++;
  }
  ...
  printf("%d characters in the text\n", printable);
  return(0);
}
